...
$result = mysqli_query($link, "SELECT * FROM giocatori WHERE id_team=\"$nome_team\"");
		...
		if ($totale != 0)
		{
			...
			{	
				$aggiorna["Nessuna"] = 70;  // abbassata da 80  a 70 perch� mi sembrava + corretto cos�... (da valutare ancora)
			...
			while ($conta < $totgio) 
			{
				if ($winfortunio[$conta] < 1)
				{
					...
					$max = 0;
					$ii = 1; 
					while   ($riga   =   mysqli_fetch_array($calc_result)) //step sulla tabella calcolatore
					{
				...
				$riquadro[$ii] = $riquadro[$ii] + $wtc[$conta]*$riga['tc'] + $wtr[$conta]*$riga['tr'];
					...
					// ASSEGNO I VALORI OTTENUTI AD UN ARRAY, COS� DA NON RIPETERE LA QUERY SUL DATABASE DEL CALCOLATORE 
