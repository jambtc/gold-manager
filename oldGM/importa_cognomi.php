...
<?php 
	...
	if (isset($_FILES['nomefile']) && is_uploaded_file($_FILES['nomefile']['tmp_name'])){
		...
		switch ($_FILES['nomefile']['error']){
			...
			case 2:
				...
				break;
			case 3:
				$errmsg_arr[] = 'il file non � stato caricato completamente';
				$errflag = true;
				break;
			case 4:
				$errmsg_arr[] = 'il file non � stato caricato';
				$errflag = true;
				break;
			case 6:
			...
			case 7:
				...
				break;
			case 8:
				$errmsg_arr[] = 'il caricamento del file � stato bloccato a causa dell\'estensione file non accettata';
				$errflag = true;
				break;
		}
	...
	if(substr($wfile,-4,4) != '.csv') {
		$errmsg_arr[] = 'Il file non � di tipo .csv';
		$errflag = true;
	}
	...
	for ($ii = 0; $ii < $new_totale; $ii++)
	{
		...
		if (!in_array($cognome,$vecchi_cognomi))
		{
			...
			$res_cognome = @mysqli_query($link, $qry_cognome);
			if (!$res_cognome)
			{
		...
		if(substr($nome,-1,1) != "a")
		{
			...
			{	
				$vecchi_nomi[] = $nome;
				$res_nome = @mysqli_query($link, $qry_nome);
				if (!$res_nome)
				{
