...
<div class="user-profile">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            ...
            <p class="text-muted-gm">Dati account e club collegato.</p>
        </div>
    ...
    <div class="row g-4">
        <div class="col-lg-6">
            <div class="gm-card h-100">
                <h3 class="h5 text-white mb-3">Account</h3>
                ...
                <div class="d-flex justify-content-between py-2">
                    <span class="text-muted-gm">Creato il</span>
        ...
        <div class="col-lg-6">
            <div class="gm-card h-100">
                <h3 class="h5 text-white mb-3">Club</h3>
                <?php if ($team): ?>
                    <div class="d-flex justify-content-between py-2 border-bottom border-secondary">
                        <span class="text-muted-gm">Squadra</span>
                        <span class="text-white fw-bold"><?= Html::encode($team->name) ?></span>
                    </div>
                    <div class="d-flex justify-content-between py-2 border-bottom border-secondary">
                        <span class="text-muted-gm">Budget</span>
                        <span class="text-gold fw-bold">EUR <?= number_format((int) $team->budget, 0, ',', '.') ?></span>
                    </div>
                    <div class="pt-3">
                        <div class="text-muted-gm small mb-2">Impostazioni club</div>
                        <?= Html::beginForm(['/user/profile'], 'post', ['class' => 'row g-2 align-items-end']) ?>
                            <div class="col-12">
                                <label class="form-label text-muted-gm small mb-1">Nome squadra</label>
                            ...
                            <div class="col-6">
                                <label class="form-label text-muted-gm small mb-1">Colore SX</label>
                            ...
                            <div class="col-6">
                                <label class="form-label text-muted-gm small mb-1">Colore DX</label>
                            ...
                            <div class="col-12">
                                <button type="submit" class="btn btn-outline-gold btn-sm mt-1">Salva club</button>
                            </div>
                        <?= Html::endForm() ?>
                    </div>
                <?php else: ?>
                    <p class="text-muted-gm mb-0">Nessuna squadra assegnata.</p>
                <?php endif; ?>
            </div>
        </div>
    </div>
...
<div class="gm-card mt-4"
    ...
    <div class="d-flex align-items-center justify-content-between mb-3">
        <h3 class="h5 text-white mb-0"><i class="bi bi-telegram text-gold me-2"></i>Notifiche Telegram</h3>
        <?php if ($tgLinked): ?>
        <div class="form-check form-switch">
            <input class="form-check-input" type="checkbox" id="tg-toggle" <?= $tgEnabled ? 'checked' : '' ?>>
            <label class="form-check-label text-muted-gm" for="tg-toggle"><?= $tgEnabled ? 'Attive' : 'Disattivate' ?></label>
        </div>
        <?php endif; ?>
    </div>
    ...
    <div class="text-muted-gm small mb-3 p-3 rounded-3" style="background:rgba(255,255,255,.03);border:1px solid var(--border)">
        <ol class="mb-0 ps-3" style="line-height:1.8">
            <li>Apri Telegram → <code>@BotFather</code> → <code>/newbot</code> → copia il token API</li>
            <li>Incolla il token e clicca <strong>1) Verifica</strong></li>
            <li>Clicca <strong>2) Genera link</strong> → <strong>Apri</strong> → invia <code>/start</code> al bot</li>
            <li>Clicca <strong>3) Conferma</strong> per registrare il chat_id</li>
            <li>Clicca <strong>4) Test</strong> per verificare la ricezione</li>
        </ol>
    </div>

    <div class="row g-3 mb-3">
        <div class="col-md-8">
            <label class="text-muted-gm small d-block mb-1">Bot token Telegram</label>
            <div class="d-flex gap-2">
                ...
                <button type="button" class="btn btn-outline-secondary btn-sm px-3" id="tg-token-show">Mostra</button>
            </div>
        </div>
        <div class="col-md-4">
            <label class="text-muted-gm small d-block mb-1">Chat ID</label>
    ...
    <div class="d-flex flex-wrap gap-2 mb-3">
        <button type="button" class="btn btn-outline-secondary btn-sm" id="tg-btn-verify">1) Verifica bot</button>
        <button type="button" class="btn btn-outline-secondary btn-sm" id="tg-btn-link-start">2) Genera link</button>
        <button type="button" class="btn btn-outline-secondary btn-sm" id="tg-btn-confirm">3) Conferma /start</button>
        <button type="button" class="btn btn-outline-secondary btn-sm" id="tg-btn-test">4) Test</button>
        <?php if ($tgLinked): ?>
        <button type="button" class="btn btn-outline-danger btn-sm" id="tg-btn-revoke">Revoca</button>
        <?php endif; ?>
    </div>

    <div id="tg-link-wrap" style="display:none" class="mb-3">
        <label class="text-muted-gm small d-block mb-1">Deep-link one-time</label>
        <div class="d-flex gap-2">
            ...
            <a href="#" id="tg-deep-link-open" class="btn btn-gold btn-sm px-3" target="_blank">Apri</a>
        </div>
    </div>

    <div id="tg-status" class="text-muted-gm small">
        <?= $tgLinked
            ? '<span style="color:var(--accent-green)">✅ Telegram collegato' . ($tgEnabled ? ' e attivo' : ' (notifiche disattivate)') . '</span>'
            : 'Usa i pulsanti sopra per collegare il tuo bot Telegram.' ?>
    </div>
</div>
